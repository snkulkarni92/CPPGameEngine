	...
	namespace Graphics
	{
		...
		bool Mesh::CreateEmpty()
		{
			...
			// Initialize the vertex format
			{
				...
				mVertexCount = 0;
				mVertexData = new sVertex[100];
			}

			// Create a vertex buffer
			{
				const unsigned int bufferSize = 100 * sizeof(sVertex);
			...
			{
				// The type of vertex processing should match what was specified when the device interface was created with CreateDevice()
				HRESULT result = GetVertexProcessingUsage(usage);
				if (FAILED(result))
				{
					return false;
				}
			...
			{
				bufferSize = 100 * sizeof(uint32_t);
				...
				const D3DPOOL useDefaultPool = D3DPOOL_DEFAULT;
				HANDLE* notUsed = NULL;
				HRESULT result = s_direct3dDevice->CreateIndexBuffer(bufferSize, usage, format, useDefaultPool,
		...
		void Mesh::AddLine(sVertex startPoint, sVertex endPoint)
		{
			mVertexData[mVertexCount] = startPoint;
			mVertexData[mVertexCount + 1] = endPoint;
			mVertexCount += 2;
		}

		void Mesh::AddMesh(LPD3DXMESH& mesh)
		{
			mesh->GetVertexBuffer(&s_vertexBuffer);
			mesh->GetIndexBuffer(&s_indexBuffer);
		}

		void Mesh::AddBox(eae6320::Math::cVector extents, eae6320::Math::cVector offset, eae6320::Math::cVector color)
		{
			LPD3DXMESH mesh;
			D3DXCreateSphere(s_direct3dDevice, 50, 10, 10, &mesh, 0);
			sVertex* vertexData1;
			HRESULT result = mesh->LockVertexBuffer(D3DLOCK_DISCARD, reinterpret_cast<void**>(&vertexData1));
			if (FAILED(result))
			{
			...
			sVertex* vertexData2;
			result = s_vertexBuffer->Lock(0, 0, reinterpret_cast<void**>(&vertexData2), 0);
			if (FAILED(result))
			{
				eae6320::UserOutput::Print("Direct3D failed to lock the vertex buffer");
			}

			for (unsigned int i = 0; i < sizeof(mesh->GetNumVertices()); i++)
			{
				vertexData2[i].x = vertexData1[i].x + offset.x;
				vertexData2[i].y = vertexData1[i].y + offset.y;
				vertexData2[i].z = vertexData1[i].z + offset.z;
				vertexData2[i].r = (uint8_t)(color.x * 255);
				vertexData2[i].g = (uint8_t)(color.y * 255);
				vertexData2[i].b = (uint8_t)(color.z * 255);
				vertexData2[i].a = 255;
				vertexData2[i].nx = vertexData1[i].nx;
				vertexData2[i].ny = vertexData1[i].ny;
				vertexData2[i].nz = vertexData1[i].nz;
				vertexData2[i].u = 0;
				vertexData2[i].v = 0;
			}
			...
			uint32_t* indexData1;
			result = mesh->LockIndexBuffer(D3DLOCK_DISCARD, reinterpret_cast<void**>(&indexData1));
			if (FAILED(result))
			{
				eae6320::UserOutput::Print("Direct3D failed to lock the vertex buffer");
			}
			...
			uint32_t* indexData2;
			result = s_indexBuffer->Lock(0, 0, reinterpret_cast<void**>(&indexData2), 0);
			if (FAILED(result))
			{
				eae6320::UserOutput::Print("Direct3D failed to lock the vertex buffer");
			}
			for (unsigned int i = 0; i < sizeof(mesh->GetNumFaces() * 3); i++)
			{
				indexData2[i] = indexData1[i];
			}
		...
		void Mesh::DrawLines()
		{
			...
			if (mVertexData)
			{
				...
				// Render objects from the current streams
				{
					...
					// We are drawing a square
					const unsigned int vertexCountToRender = mVertexCount;	// How vertices from the vertex buffer will be used?
					result = s_direct3dDevice->DrawPrimitive(primitiveType,
						indexOfFirstVertexToRender, vertexCountToRender);
					assert(SUCCEEDED(result));
				}
				mVertexCount = 0;
			}
